...
if ($method === 'GET') {
    ...
    try {
        ...
        // Inserisci vendita
        $stmt = $db->prepare("
            INSERT INTO sales (total, discount_type, discount_value, payment_method, user_id)
            VALUES (?, ?, ?, ?, ?)
        ");
        $stmt->execute([$total, $discount_type, $discount_value, $payment_method, $_SESSION['user_id']]);
        ...
        foreach ($items as $item) {
            ...
            $inv_stmt = $db->prepare("
                INSERT INTO inventory_movements (product_id, type, qty, unit_type, reason, user_id)
                VALUES (?, 'out', ?, ?, 'vendita', ?)
            ");
            $inv_stmt->execute([$item['product_id'], $item['qty'], $item['unit_type'], $_SESSION['user_id']]);
        }
